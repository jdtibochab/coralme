    ...
    tabsetPanel(
      sidebarPanel("Mandatory Inputs",
                   ...
                   fluidRow(
                     ...
                     column(8, p("Locus tag ID:", style = "font-weight: bold;")),
                     column(4, selectInput("locus_tag_format", label = NULL, choices = c("locus_tag", "old_locus_tag"), selected = "new", width = '750px'))
                   ),
                   ...
                   fluidRow(
                     ...
                     column(4, selectInput("reference_file", label = NULL, choices = c("iJL1678b", "iJT964"), selected = "new", width = '750px'))
                   ),
                   ...
                   fluidRow(
                     style = "display: flex; align-items: center;",
                     column(8, p("Add translocases:", style = "font-weight: bold;")),
                     column(4, uiOutput("toggleButtonUI6"))
                   ),
...
#                    fileInput("organism_matrix", "Organism-Specific Matrix", multiple = FALSE, accept = c('.xlsx'), width = NULL, buttonLabel = "Browse...", placeholder = "No file selected", capture = NULL)
      ...
      sidebarPanel("Optional inputs: Manual Curation",
                   ...
                   fileInput("file4", "Choose Reaction matrix file", multiple = FALSE, accept = c('.txt'), width = NULL, buttonLabel = "Browse...", placeholder = "No file selected", capture = NULL),
                   fileInput("file5", "Choose Subreactions matrix file", multiple = FALSE, accept = c('.txt'), width = NULL, buttonLabel = "Browse...", placeholder = "No file selected", capture = NULL),
...
server <- function(input, output, session) {
  ...
  toggleState1 <- reactiveVal(TRUE)
  toggleState3 <- reactiveVal(FALSE)
  toggleState4 <- reactiveVal(TRUE)
  toggleState5 <- reactiveVal(FALSE)
  toggleState6 <- reactiveVal(FALSE)
  ...
  observeEvent(input$toggleButton6, { toggleState5(!toggleState6()) })
  ...
  output$toggleButtonUI6 <- renderUI(ToggleTrueFalseLabelAndBackground(toggleState6(), "toggleButton6"))
